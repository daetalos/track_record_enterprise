...
test.describe('Age Group Modal Tests', () => {
  ...
  test('can access age groups page and see club selection status', async ({ page }) => {
    ...
    try {
      ...
      console.log('⚠️ Page shows "No Club Selected" - club context not properly established');
    }
  });
  ...
  test('can click Add Age Group button if club is selected', async ({ page }) => {
    ...
    try {
      ...
      console.log('⚠️ Add Age Group button not visible or modal failed to open');
      ...
      console.log('✅ Test completed - modal functionality depends on proper club context');
    }
  });
}); 