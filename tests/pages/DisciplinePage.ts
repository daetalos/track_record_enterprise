...
export class DisciplinePage {
  ...
  constructor(page: Page) {
    ...
    this.modalTitle = page.getByRole('heading', {
      name: /(create|edit) discipline/i,
    });
    this.modalDescription = page.getByText(
      /(add a new discipline|update the discipline)/i
    );
    ...
    this.measuredLabel = page.getByText(
      /measured.*events measured by distance/i
    );
    ...
    this.submitButton = page.getByRole('button', {
      name: /(create|update) discipline/i,
    });
    this.createSubmitButton = page.getByRole('button', {
      name: /create discipline/i,
    });
    this.updateSubmitButton = page.getByRole('button', {
      name: /update discipline/i,
    });
  ...
  async expectEmptyState(seasonName?: string) {
    if (seasonName) {
      await expect(this.emptyStateTitle).toContainText(
        `No Disciplines in ${seasonName}`
      );
      await expect(this.emptyStateMessage).toContainText(
        "doesn't have any disciplines yet"
      );
    } else {
  ...
  async expectResultsCount(count: number) {
    await expect(this.resultsInfo).toContainText(
      `Showing ${count} of ${count} discipline`
    );
  }
