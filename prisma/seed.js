  ...
  try {
    ...
    for (const rel of data.relationships) {
      if (rel.user_index >= data.users.length || rel.user_index < 0) {
        throw new Error(`Invalid user_index ${rel.user_index} in relationships`);
      }
      if (rel.club_index >= data.clubs.length || rel.club_index < 0) {
        throw new Error(`Invalid club_index ${rel.club_index} in relationships`);
      }
    ...
    console.log(`📄 Loaded seed data v${data.metadata.version}: ${data.clubs.length} clubs, ${data.users.length} users, ${data.relationships.length} relationships`);
    return data;
...
async function main() {
  ...
  if (!isDevelopment) {
    console.log('⚠️  Seeding skipped - only allowed in development environment');
    return;
  }
  ...
  seedData.users.forEach(user => {
    console.log(`   📧 ${user.email} (password: password123) - ${user.role_description}`);
  });
